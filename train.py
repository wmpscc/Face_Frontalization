...
parser.add_argument("-c","--continue_training", action='store_true', help='continue training', default=False)
parser.add_argument("-G_xvz", "--G_xvz_name", type=str, default="netG_xvz_epoch_24_699.pth")
parser.add_argument("-G_vzx", "--G_vzx_name", type=str, default="netG_vzx_epoch_24_699.pth")
parser.add_argument("-D_xvs", "--D_xvs_name", type=str, default="netD_xvs_epoch_24_699.pth")
parser.add_argument('--id_loss', action='store_true', help='add id preserving loss', default=False)
parser.add_argument("-id_w", "--id_weight", type=int, default=20)
parser.add_argument('--sym_loss', action='store_true', help='add symmetry loss', default=False)
parser.add_argument("-sym_w", "--symmetry_loss_weight", type=int, default=0.1)
parser.add_argument("-s","--save", action='store_true', help='save good&bad imgs', default=False)
...
for epoch in range(args.epochs):
    for i, (view1, view2, data1, data2) in enumerate(train_loader):
        ...
        # path 1, update D_xvs
        x_bar = G_vzx(v1, z, ID_r) # random z to generate img x_bar
        ...
        LightCNN = ID_pre.define_R(gpu_ids=[0, 1, 2, 3, 4, 5, 6, 7], lightcnn_path='/home/hejingwei/whj/F06All/ID_models/LightCNN_29Layers_V2_checkpoint.pth').cuda()
        ...
        if reconstruct_fake is True: 
            ...
            x_bar_bar = G_vzx(v1, z_bar, x_bar_ID_fea)
            ...
            x_bar_bar = G_vzx(v2, z_bar, x1_ID_fea) # x_bar_bar: reconstruction of x2
        ...
        if reconstruct_fake is False:
            # id-preserving, loss
            l_g_rec = ID_weight * cri_rec(fake_fea, real_fea, Variable(torch.ones((real_fea.shape[0], 1))).cuda()) 
            # add symmetry loss 
        ...
        if reconstruct_fake is False:
            print("Epoch: [%2d] [%4d/%4d] time: %4.2f, ""loss_D_vx: %.2f, loss_D_x: %.2f, loss_G: %.2f, ID_loss: %.2f, sym_loss: %.2f"% (epoch, i, len(data1), time.time() - start_time, d_xvs_loss.data[0], d_x_loss.data[0], (g_loss.data[0]-l_g_rec.data[0]), l_g_rec.data[0], symmetry_128_loss.data[0]))
            loss_str = "Epoch: " + '[' + str(epoch) + ']' + '[' + str(i)+ '/'+ str(len(data1)) + ']' + " loss_D_vx: " + str(round(d_xvs_loss.data[0], 3)) +" loss_D_x: " + str(round(d_x_loss.data[0], 3)) + " loss_G: " + str(round((g_loss.data[0]-l_g_rec.data[0]), 3)) + " l_g_rec: " + str(round(l_g_rec.data[0], 3)) + str(round(symmetry_128_loss.data[0], 3)) + "\n"
        else:
            print("Epoch: [%2d] [%4d/%4d] time: %4.2f, ""loss_D_vx: %.2f, loss_D_x: %.2f, loss_G: %.2f"% (epoch, i, len(data1), time.time() - start_time,d_xvs_loss.data[0], d_x_loss.data[0],  g_loss.data[0] ))
            loss_str = "Epoch: " + '[' + str(epoch) + ']' + '[' + str(i)+ '/'+ str(len(data1)) + ']' + " loss_D_vx: " + str(round(d_xvs_loss.data[0], 3)) +" loss_D_x: " + str(round(d_x_loss.data[0], 3)) + " loss_G: " + str(round(g_loss.data[0], 3)) + "\n"
        fout.write(loss_str)
